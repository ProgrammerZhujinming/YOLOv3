...
class VOCDataSet(Dataset):
    def __init__(self, imgs_path="../DataSet/VOC2007+2012/Train/JPEGImages", annotations_path="../DataSet/VOC2007+2012/Train/Annotations", classes_file="../DataSet/VOC2007+2012/class.data", is_train=True, class_num=20, iou_threshold=0.225, label_smooth_value=0.05, small_downsample=8, middle_downsample=16, big_downsample=32): #input_size:输入图像的尺度
    ...
    def getGroundTruth(self, coords):
        ...
        # positive modulus
        small_positive_modulus = np.zeros([small_feature_size, small_feature_size, 3, 5])
        middle_positive_modulus = np.zeros([middle_feature_size, middle_feature_size, 3, 5])
        big_positive_modulus = np.zeros([big_feature_size, big_feature_size, 3, 5])

        # modulus mask
        small_positive_modulus_mark = np.zeros([small_feature_size, small_feature_size, 3, 5])
        middle_positive_modulus_mark = np.zeros([middle_feature_size, middle_feature_size, 3, 5])
        big_positive_modulus_mark = np.zeros([big_feature_size, big_feature_size, 3, 5])
        ...
        for coord in coords:
            ...
            for anchor_index in range(len(self.anchors_size)):
                ...
                if iou > self.iou_threshold:
                    ...
                    if anchor_index < 3 and small_anchor_mark_positive[small_indexRow][small_indexCol][anchor_index][0] != 0:
                        ...
                        #temp_negative_sample.append([small_indexRow, small_indexCol, anchor_index])
                        ...
                        #temp_negative_sample.append([small_indexRow, small_indexCol, anchor_index % 3])
            ...
            modulus = [scale_adjust_modulu, self.input_size * xmin, self.input_size * ymin, self.input_size * xmax, self.input_size * ymax]
            ...
            if max_iou_index < 3:
                ...
                small_positive_modulus_mark[small_indexRow][small_indexCol][max_iou_index] = np.ones([5])
                ...
                middle_positive_modulus_mark[middle_indexRow][middle_indexCol][max_iou_index- 3] = np.ones([5])
                ...
                big_positive_modulus_mark[big_indexRow][big_indexCol][max_iou_index - 6] = np.ones([5])
